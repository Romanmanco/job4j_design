...
public class Find {
    ...
    private static void checkArgs(String[] args) {
        if (args.length != 4
                ...
                || !args[2].startsWith("-t=")
                || !args[3].startsWith("-o=")) {
            throw new IllegalArgumentException("Wrong parameters. Usage "
                    + "-d=\"c://Users//User//X\" "
                    + "-n=\"*.txt\" "
                    + "-t=mask "
                    + "-o=\"resultFileFinder.txt\"\n "
    ...
    private static FinderFiles findOption(ArgsName argsName) {
        ...
        if ("mask".equals(argsName.get("t"))) {
            ...
            return new FinderFiles(e -> pattern.matcher(e).find());
        }

        return new FinderFiles(e -> e.endsWith(argsName.get("n")));
    }
    ...
    public static void search(ArgsName argsName) throws IOException {
        ...
        FinderFiles searchType = findOption(argsName);
